...
from xhs_harness import default_agent_dir, default_config_path, default_media_root, find_openclaw_dist, probe_describe_runtime, DEFAULT_VISION_MODEL
...
def main():
    ...
    def add(name, ok, **extra):
        ...
        if not ok: errors.append(extra.get('error') or f'{name} failed')
    ...
    if node:
        ...
        except Exception as e: add('node', False, path=node, error=str(e))
    else: add('node', False, error='node not found in PATH')
