...
from xhs_harness import default_media_root
...
def safe_root(path: Path) -> Path:
    ...
    if 'xiaohongshu' not in root.parts:
        raise ValueError(f'unsafe root missing xiaohongshu segment: {root}')
    return root
    ...
    try:
        ...
        if root.exists():
            for p in sorted(root.rglob('*'), key=lambda x: len(x.parts), reverse=True):
                ...
                except Exception as e: warnings.append(f'file_skip {p}: {e}')
            for p in sorted([x for x in root.rglob('*') if x.is_dir()], key=lambda x: len(x.parts), reverse=True):
                try:
                    if not any(p.iterdir()):
                        deleted_dirs.append(str(p))
                        if not args.dry_run: p.rmdir()
                except Exception as e: warnings.append(f'dir_skip {p}: {e}')
        else:
