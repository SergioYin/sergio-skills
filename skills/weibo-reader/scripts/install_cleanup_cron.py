...
def cleanup_message(days: int):
    return f"Run exactly this local cleanup command and report only the JSON result: cd ~/.openclaw/workspace/skills/weibo-reader && python3 scripts/cleanup_media.py --days {days} --json"
...
def main():
    ...
    ap.add_argument('--cron', default='15 3 * * *')
    ...
    try:
        jobs, _ = list_jobs(); matches = find_jobs(jobs)
            ...
            for j in matches:
                ...
                if not args.dry_run:
                    ...
                    try:
                        ...
                        actions[-1]['result'] = p.stdout
        jobs2, _ = list_jobs(); matches2 = find_jobs(jobs2)
